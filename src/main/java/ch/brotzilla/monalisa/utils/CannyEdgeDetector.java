...
public class CannyEdgeDetector {
    ...
    private static BufferedImage readImage(String file) throws IOException {
        final BufferedImage result = ImageIO.read(new File(file));
        if (result.getType() != BufferedImage.TYPE_INT_ARGB) {
            final BufferedImage conv = new BufferedImage(result.getWidth(), result.getHeight(), BufferedImage.TYPE_INT_ARGB);
            final Graphics2D g = (Graphics2D) conv.getGraphics();
    ...
    public static void main(String[] args) throws IOException {
        final CannyEdgeDetector ced = new CannyEdgeDetector();
        ced.setSourceImage(readImage("./data/pictures/SchatzUndIch.png"));
        ced.setContrastNormalized(false);
        ced.setLowThreshold(1.5f);
        ced.setHighThreshold(3.5f);
        ced.process();
        ImageIO.write(ced.getEdgesImage(), "PNG", new File("./test.png"));
    }
