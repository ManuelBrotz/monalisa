	...
	public final boolean nextBoolean(final float probability) {
		...
		if (probability < 0.0f || probability > 1.0f)
			throw new IllegalArgumentException(
					"probability must be between 0.0 and 1.0 inclusive."); //$NON-NLS-1$
		if (this.mti >= N) // generate N words at one time
		{
			int kk;
	...
	public final void nextBytes(byte[] bytes) {
		...
		for (int x = 0; x < bytes.length; x++) {
			if (this.mti >= N) // generate N words at one time
			{
				...
				for (kk = 0; kk < N - M; kk++) {
					y = (this.mt[kk] & UPPER_MASK)
							| (this.mt[kk + 1] & LOWER_MASK);
					this.mt[kk] = this.mt[kk + M] ^ (y >>> 1)
							^ this.mag01[y & 0x1];
				}
				for (; kk < N - 1; kk++) {
					y = (this.mt[kk] & UPPER_MASK)
	...
	public final double nextGaussian() {
		...
		do {
			...
			if (this.mti >= N) // generate N words at one time
			{
				...
				for (kk = 0; kk < N - M; kk++) {
					z = (this.mt[kk] & UPPER_MASK)
							| (this.mt[kk + 1] & LOWER_MASK);
					this.mt[kk] = this.mt[kk + M] ^ (z >>> 1)
							^ this.mag01[z & 0x1];
				}
				for (; kk < N - 1; kk++) {
					z = (this.mt[kk] & UPPER_MASK)
			...
			if (this.mti >= N) // generate N words at one time
			{
				...
				for (kk = 0; kk < N - M; kk++) {
					a = (this.mt[kk] & UPPER_MASK)
							| (this.mt[kk + 1] & LOWER_MASK);
					this.mt[kk] = this.mt[kk + M] ^ (a >>> 1)
							^ this.mag01[a & 0x1];
				}
				for (; kk < N - 1; kk++) {
					a = (this.mt[kk] & UPPER_MASK)
							| (this.mt[kk + 1] & LOWER_MASK);
					this.mt[kk] = this.mt[kk + (M - N)] ^ (a >>> 1)
							^ this.mag01[a & 0x1];
				}
				a = (this.mt[N - 1] & UPPER_MASK) | (this.mt[0] & LOWER_MASK);
				this.mt[N - 1] = this.mt[M - 1] ^ (a >>> 1)
						^ this.mag01[a & 0x1];
			...
			if (this.mti >= N) // generate N words at one time
			{
				...
				for (kk = 0; kk < N - M; kk++) {
					b = (this.mt[kk] & UPPER_MASK)
							| (this.mt[kk + 1] & LOWER_MASK);
					this.mt[kk] = this.mt[kk + M] ^ (b >>> 1)
							^ this.mag01[b & 0x1];
				}
				for (; kk < N - 1; kk++) {
					b = (this.mt[kk] & UPPER_MASK)
							| (this.mt[kk + 1] & LOWER_MASK);
					this.mt[kk] = this.mt[kk + (M - N)] ^ (b >>> 1)
							^ this.mag01[b & 0x1];
				}
				b = (this.mt[N - 1] & UPPER_MASK) | (this.mt[0] & LOWER_MASK);
				this.mt[N - 1] = this.mt[M - 1] ^ (b >>> 1)
						^ this.mag01[b & 0x1];
